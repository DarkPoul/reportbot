...
@Slf4j
@Component
public class TaxiStepHandler implements StepHandler {

    private final MessageSender messageSender;
    private final InputValidator inputValidator;

    public TaxiStepHandler(MessageSender messageSender, InputValidator inputValidator) {
    ...
    public List<UserSession.Step> supportedSteps() {
        return List.of(UserSession.Step.WAIT_TAXI);
    }
    ...
    public void handle(long userId, String chatId, String text, UserSession session) {
        ...
        session.getReportDraft().setDeliveryAmount(amount.get());
        session.setStep(UserSession.Step.WAIT_COLLECTION);
        messageSender.sendText(chatId, BotMessages.ASK_COLLECTION);
    }
}
