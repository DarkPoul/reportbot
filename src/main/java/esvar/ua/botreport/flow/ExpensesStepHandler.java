...
@Slf4j
@Component
public class ExpensesStepHandler implements StepHandler {

    private final MessageSender messageSender;
    private final InputValidator inputValidator;

    public ExpensesStepHandler(MessageSender messageSender, InputValidator inputValidator) {
    ...
    public List<UserSession.Step> supportedSteps() {
        return List.of(UserSession.Step.WAIT_CASH_EXPENSES);
    }
    ...
    public void handle(long userId, String chatId, String text, UserSession session) {
        ...
        session.getReportDraft().setExpenses(amount.get());
        session.setStep(UserSession.Step.WAIT_NEXT_NAME);
